...
public class EndGame extends MainActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        double subTime;
        if (partTime.equals("0:") && secondTime < 10 && secondTime != 0) {
            subTime = 0.1 * secondTime;
        } else if (partTime.equals("0:") && secondTime >= 10) {
            subTime = 0.1 * secondTime;
    ...
    private void difficulty() {
        ...
        if (!loser) {
            gameOver = findViewById(R.id.gameOverWin);
            time = findViewById(R.id.endTimeWin);
            score = findViewById(R.id.endScoreWin);
            endGame = findViewById(R.id.levelCompleteWin);
            gameOver.setVisibility(View.VISIBLE);
            time.setVisibility(View.VISIBLE);
            score.setVisibility(View.VISIBLE);
            endGame.setVisibility(View.VISIBLE);
        } else {
            gameOver = findViewById(R.id.gameOverLose);
            time = findViewById(R.id.endTimeLose);
            score = findViewById(R.id.endScoreLose);
            endGame = findViewById(R.id.levelCompleteLose);
            gameOver.setVisibility(View.VISIBLE);
            time.setVisibility(View.VISIBLE);
            score.setVisibility(View.VISIBLE);
            endGame.setVisibility(View.VISIBLE);
